...
class TestBuildImmutableRepository(unittest.TestCase):
    ...
    def test_duplicate_addon_ids_are_rejected_as_ambiguous(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            site = root / "_site"
    ...
    def test_malformed_matching_addon_missing_version_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            site = root / "_site"
            ...
            existing_addon_xml = (
                "<addons>"
                '<addon id="plugin.video.twitch" name="Twitch" '
                'provider-name="test"/>'
                "</addons>"
            )
            existing_manifest_bytes = existing_addon_xml.encode("utf-8")
            ...
            error = str(ctx.exception)
            self.assertIn("plugin.video.twitch", error)
            self.assertIn("no version", error)
            self.assertEqual(_snapshot_tree(repo), repo_before)
            self.assertEqual(_snapshot_tree(site), site_before)

    def test_malformed_matching_addon_invalid_version_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            site = root / "_site"
            ...
            existing_addon_xml = (
                "<addons>"
                '<addon id="plugin.video.twitch" name="Twitch" version="3.1.x" '
                'provider-name="test"/>'
                "</addons>"
            )
            existing_manifest_bytes = existing_addon_xml.encode("utf-8")
    ...
    def test_unexpected_root_element_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            site = root / "_site"
    ...
    def test_non_addon_element_before_matching_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            site = root / "_site"
            ...
            existing_addon_xml = (
                "<addons>"
                '<not-an-addon id="plugin.other" version="1.0.0"/>'
                '<addon id="plugin.video.twitch" name="Twitch" version="3.1.10" '
                'provider-name="test"/>'
                "</addons>"
            )
